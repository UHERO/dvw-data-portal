...
export class DvwApiService {
  ...
  getDimensions(mod: string): any {
    if (this.cachedDimensions[mod]) {
      ...
      let moduleDimensions$ = this.http
        .get(`${API_URL}/dimensions/${mod}`)
        .pipe(
          map(this.mapData),
          tap((val) => {
  ...
  getOptions(dimension: string, mod: string): Observable<Array<any>> {
    if (this.cachedDimensionOptions[mod]) {
      ...
      let moduleDimensionOptions$ = this.http
        .get(`${API_URL}/${dimension}/all/${mod}`)
        .pipe(
          map(this.mapData),
          tap((val) => {
    ...
    if (this.cachedDimensionOptions[mod]) {
      this.cachedDimensionOptions[mod].forEach((dim) => {
        ...
        selected.forEach((option) => {
          dim.options.find((opt) => opt.handle === option.handle).selected =
            false;
        });
        active.forEach((option) => {
          dim.options.find((opt) => opt.handle === option.handle).active =
            false;
        });
      });
      ...
      let moduleDimensionOptions$ = this.http
        ...
        .pipe(
          ...
          mergeMap((dimensions) =>
            observableForkJoin(
              dimensions.map((d) =>
                this.http.get(`${API_URL}/${d}/all/${mod}`).pipe(
                  map((res: any) => {
  ...
  getFrequencies(mod: string, dimensions: string) {
    ...
    if (this.cachedFrequencies[`${mod}:${dimensions}`]) {
      ...
      let freqs$ = this.http
        .get(`${API_URL}/freqavail/${mod}?${dimensions}`)
        .pipe(
      ...
      console.log(this.cachedFrequencies, "freqs");
      return freqs$;
    }
  }
  ...
  getSeries(mod: string, dimensionList: string, freq: string) {
    ...
    if (this.cachedSeries[`${mod}:${dimensionList}:${freq}`]) {
      ...
      let series$ = this.http
        .get(`${API_URL}/series/${mod}?${dimensionList}&f=${freq}`)
        .pipe(
          map(this.mapData),
          tap((val) => {
  ...
  mapDimensionOrder = (response: any, mod: string): any => {
    ...
    const order = this._helper.dimensionsOrder.find(
